            ...
            buttons: [
                {
                    ...
                    action: function () {
                        $.LoadingOverlay("show");
                        window.location.href = '/Administration/FormsSpedycjaStandardyPrzygotowania/Create';
                    }
                },
                {
                    extend: 'print',
                    ...
                    buttons: [
                        {
                            text: 'Cała baza',
                            action: function () {
                                window.location.href = '/Administration/FormsSpedycjaStandardyPrzygotowania/ExportExcel';
                            }
                        },
                        {
                            extend: 'excel',
...
(function ($) {
    $('#datatable').on("click", '#modify-button', function () {
        ...
        var id = $(this).data("id");
        $.ajax({
            url: "/Administration/FormsSpedycjaStandardyPrzygotowania/Create",
            data: { id: id },
    ...
    $('#datatable').on("click", '#delete-button', function () {
        ...
        var id = $(this).data("id");
        $.ajax({
            url: "/Administration/FormsSpedycjaStandardyPrzygotowania/Delete",
            data: { id: id },
            success: function (response) {
                $.LoadingOverlay("hide");
                $("#modal-content").html(response);
