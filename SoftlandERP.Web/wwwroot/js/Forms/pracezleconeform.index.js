    ...
    var datatableInit = function () {
        ...
        var fnFormatDetails = function (datatable, tr) {
            ...
            return [
                ...
                '<tr class="b-top-0">',
                '<td style="width: 200px;"><label class="mb-0">Wartość pracy zleconej [PLN]: </label></td>',
                '<td>' + data[14] + '</td>',
                '</tr>',
                '<tr>',
            ...
            buttons: [
                {
                    ...
                    action: function () {
                        $.LoadingOverlay("show");
                        window.location.href = '/Forms/PraceZleconeForm/Create';
                        $.LoadingOverlay("hide");
                    }
                },
                {
                    ...
                    buttons: [
                        {
                            text: 'Cała baza',
                            action: function () {
                                window.location.href = '/Forms/PraceZleconeForm/ExportExcel';
                            }
                        },
                        {
                            extend: 'excel',
...
(function ($) {
    $('#datatable').on("click", '#modify-button', function () {
        ...
        $.ajax({
            ...
            url: "/Forms/PraceZleconeForm/Create",
            data: { id: id },
        });
    });
    $('#datatable').on("click", '#delete-button', function () {
        ...
        var id = $(this).data("id");
        $.ajax({
            url: "/Forms/PraceZleconeForm/Delete",
            data: { id: id },
    ...
    $('#datatable').on("click", '#clone-button', function () {
        ...
        var id = $(this).data("id");
        $.ajax({
            url: "/Forms/PraceZleconeForm/Clone",
            data: { id: id },
            success: function (response) {
                $.LoadingOverlay("hide");
                $("#modal-content").html(response);
