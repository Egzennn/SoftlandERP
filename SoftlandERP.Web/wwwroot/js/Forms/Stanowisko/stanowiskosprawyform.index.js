    ...
    var datatableInit = function () {
        ...
        var fnFormatDetails = function (datatable, tr) {
            ...
            return [
                ...
                '<tr class="b-top-0">',
                '<td style="width: 200px;"><label class="mb-0">Temat:</label></td>',
                '<td>' + data[6] + '</td>',
                '</tr>',
                '<tr class="b-top-0">',
                ...
                '</tr>',
                '<tr class="b-top-0">',
                '<td style="width: 200px;"><label class="mb-0">Procent realizacji:</label></td>',
                '<td>' + data[10] + '</td>',
                '</tr>',
                '<tr class="b-top-0">',
                '<td style="width: 200px;"><label class="mb-0">Czas pracy:</label></td>',
                '<td>' + data[11] + '</td>',
                '</tr>',
                '<tr>',
            ...
            buttons: [
                {
                    ...
                    action: function () {
                        $.LoadingOverlay("show");
                        $.ajax({
                            url: "/Forms/StanowiskoSprawyForm/Create",
                    ...
                    buttons: [
                        {
                            text: 'Cała baza',
                            action: function () {
                                window.location.href = '/Forms/StanowiskoSprawyForm/ExportExcel';
                            }
                        },
                        {
                            extend: 'excel',
...
(function ($) {
    ...
    $('#datatable').on("click", '#delete-button', function () {
        ...
        var id = $(this).data("id");
        $.ajax({
            url: "/Forms/StanowiskoSprawyForm/Delete",
            data: { id: id },
            success: function (response) {
                $.LoadingOverlay("hide");
                $("#modal-content").html(response);
    ...
    $('#datatable').on("click", '#clone-button', function () {
        ...
        var id = $(this).data("id");
        $.ajax({
            url: "/Forms/StanowiskoSprawyForm/Clone",
            data: { id: id },
            success: function (response) {
                $.LoadingOverlay("hide");
                $("#modal-content").html(response);
