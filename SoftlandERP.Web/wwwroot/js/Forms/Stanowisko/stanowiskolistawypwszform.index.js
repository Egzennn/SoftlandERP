    ...
    var datatableInit = function () {
        ...
        var fnFormatDetails = function (datatable, tr) {
            ...
            return [
                ...
                '<tr class="b-top-0">',
                '<td style="width: 200px;"><label class="mb-0">Grupa produktu:</label></td>',
                '<td>' + data[6] + '</td>',
                '</tr>',
                '<tr>',
            ...
            buttons: [
                {
                    ...
                    action: function () {
                        $.LoadingOverlay("show");
                        $.ajax({
                            url: "/Forms/StanowiskoListaWypWSZForm/Create",
                    ...
                    buttons: [
                        {
                            text: 'Cała baza',
                            action: function () {
                                window.location.href = '/Forms/StanowiskoListaWypWSZForm/ExportExcel';
                            }
                        },
                        {
                            extend: 'excel',
...
(function ($) {
    ...
    $('#datatable').on("click", '#delete-button', function () {
        ...
        var id = $(this).data("id");
        $.ajax({
            url: "/Forms/StanowiskoListaWypWSZForm/Delete",
            data: { id: id },
            success: function (response) {
                $.LoadingOverlay("hide");
                $("#modal-content").html(response);
    ...
    $('#datatable').on("click", '#clone-button', function () {
        ...
        var id = $(this).data("id");
        $.ajax({
            url: "/Forms/StanowiskoListaWypWSZForm/Clone",
            data: { id: id },
            success: function (response) {
                $.LoadingOverlay("hide");
                $("#modal-content").html(response);
