    ...
    var datatableInit = function () {
        ...
        var fnFormatDetails = function (datatable, tr) {
            ...
            return [
                ...
                '<tr class="b-top-0">',
                '<td style="width: 200px;"><label class="mb-0">Login:</label></td>',
                '<td>' + data[6] + '</td>',
                '</tr>',
                '<tr class="b-top-0">',
            ...
            buttons: [
                {
                    ...
                    action: function () {
                        $.LoadingOverlay("show");
                        $.ajax({
                            url: "/Forms/StanowiskoObiektyITForm/Create",
                    ...
                    buttons: [
                        {
                            text: 'Cała baza',
                            action: function () {
                                window.location.href = '/Forms/StanowiskoObiektyITForm/ExportExcel';
                            }
                        },
                        {
                            extend: 'excel',
...
(function ($) {
    ...
    $('#datatable').on("click", '#delete-button', function () {
        ...
        var id = $(this).data("id");
        $.ajax({
            url: "/Forms/StanowiskoObiektyITForm/Delete",
            data: { id: id },
            success: function (response) {
                $.LoadingOverlay("hide");
                $("#modal-content").html(response);
    ...
    $('#datatable').on("click", '#clone-button', function () {
        ...
        var id = $(this).data("id");
        $.ajax({
            url: "/Forms/StanowiskoObiektyITForm/Clone",
            data: { id: id },
            success: function (response) {
                $.LoadingOverlay("hide");
                $("#modal-content").html(response);
