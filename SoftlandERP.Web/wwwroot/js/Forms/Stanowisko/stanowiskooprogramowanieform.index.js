            ...
            buttons: [
                {
                    ...
                    action: function () {
                        $.LoadingOverlay("show");
                        $.ajax({
                            url: "/Forms/StanowiskoOprogramowanieForm/Create",
                    ...
                    buttons: [
                        {
                            text: 'Cała baza',
                            action: function () {
                                window.location.href = '/Forms/StanowiskoOprogramowanieForm/ExportExcel';
                            }
                        },
                        {
                            extend: 'excel',
...
(function ($) {
    ...
    $('#datatable').on("click", '#delete-button', function () {
        ...
        var id = $(this).data("id");
        $.ajax({
            url: "/Forms/StanowiskoOprogramowanieForm/Delete",
            data: { id: id },
            success: function (response) {
                $.LoadingOverlay("hide");
                $("#modal-content").html(response);
    ...
    $('#datatable').on("click", '#clone-button', function () {
        ...
        var id = $(this).data("id");
        $.ajax({
            url: "/Forms/StanowiskoOprogramowanieForm/Clone",
            data: { id: id },
            success: function (response) {
                $.LoadingOverlay("hide");
                $("#modal-content").html(response);
