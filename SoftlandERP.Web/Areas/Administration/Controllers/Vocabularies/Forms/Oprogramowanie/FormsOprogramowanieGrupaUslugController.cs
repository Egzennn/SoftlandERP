...
namespace SoftlandERP.Web.Areas.Administration.Controllers.Vocabularies.Forms.Oprogramowanie
{
    [Area("Administration")]
    public class FormsOprogramowanieGrupaUslugController : BaseController
    {
        public static readonly string Module = "Słownik - ";
        public static readonly string Name1 = "Oprogramowanie - ";
        public static readonly string Name2 = "Rola";
        public static readonly string Name = Name1 + Name2;
        public static readonly string ModuleName = Module + Name;

        private readonly IRepository<OprogramowanieGrupaUslug> repository;
        private readonly IRepository<OgolneStan> stanRepository;

        public FormsOprogramowanieGrupaUslugController(IADRepository adRepository, IRepository<HelperText> helperTextVocabularyRepository, IToastNotification toastNotification, ILogger<BaseController> logger, IRepository<OprogramowanieGrupaUslug> repository, IRepository<OgolneStan> stanRepository)
        ...
        public IActionResult Create(Guid? id)
        {
            try
            {
                ...
                return this.PartialView("Modals/Create", this.repository.GetByIdAsync(id).Result ?? new OprogramowanieGrupaUslug());
            }
            catch (Exception ex)
            {
                this.toastNotification.AddErrorToastMessage("Brak połączenia z bazą danych");
        ...
        public IActionResult Create(OprogramowanieGrupaUslug model)
        {
            try
            {
                if (!this.CheckPermission(ModuleName))
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public IActionResult Delete(OprogramowanieGrupaUslug model)
        {
            try
            {
                if (!this.CheckPermission(ModuleName))
        ...
        public IActionResult ExportExcel()
        {
            try
            {
                return ExcelExporter.Export(this.repository.GetAllAsync().Result?.ToList() ?? new List<OprogramowanieGrupaUslug>(), ModuleName);
            }
            catch
            {
                this.toastNotification.AddErrorToastMessage("Brak połączenia z bazą danych");
