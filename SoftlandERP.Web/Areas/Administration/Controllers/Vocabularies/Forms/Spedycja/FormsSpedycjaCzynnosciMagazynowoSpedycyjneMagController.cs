...
{
    [Area("Administration")]
    public class FormsSpedycjaCzynnosciMagazynowoSpedycyjneMagController : BaseController
    {
        public static readonly string Module = "Słownik - ";
        public static readonly string Name = "Spedycja - Czynności Magazynowo-Spedycyjne Mag";
        public static readonly string ModuleName = Module + Name;

        private readonly IRepository<SpedycjaCzynnosciMagazynowoSpedycyjneMag> repository;
        private readonly IRepository<OgolneStan> stanRepository;
        private readonly IRepository<SpedycjaCzynnosci> czynnosciRepository;
        private readonly IRepository<OgolneKategoriaPracy> rodzajepracRepository;

        public FormsSpedycjaCzynnosciMagazynowoSpedycyjneMagController(IADRepository adRepository, IRepository<HelperText> helperTextVocabularyRepository, IToastNotification toastNotification, ILogger<BaseController> logger, IRepository<SpedycjaCzynnosciMagazynowoSpedycyjneMag> repository, IRepository<OgolneStan> stanRepository, IRepository<SpedycjaCzynnosci> czynnosciRepository, IRepository<OgolneKategoriaPracy> rodzajepracRepository)
            : base(adRepository, helperTextVocabularyRepository, toastNotification, logger)
        {
            this.repository = repository;
            this.stanRepository = stanRepository;
            this.czynnosciRepository = czynnosciRepository;
            this.rodzajepracRepository = rodzajepracRepository;
        }
        ...
        public IActionResult Create(Guid? id)
        {
            try
            {
                ...
                this.ViewBag.Odpowiedzialny = new SelectList(this.adRepository.GetAllADUserAcronyms());
                this.ViewBag.Czynnosci = new SelectList(this.czynnosciRepository.GetAllAsync().Result?.Where(x => x.Stan == "Aktywny").OrderBy(x => x.Wartosc).Select(x => x.Wartosc).ToList() ?? new List<string?>());
                this.ViewBag.RodzajePrac = new SelectList(this.rodzajepracRepository.GetAllAsync().Result?.Where(x => x.Stan == "Aktywny").OrderBy(x => x.Wartosc).Select(x => x.Wartosc).ToList() ?? new List<string?>());
                ...
                return this.PartialView("Modals/Create", this.repository.GetByIdAsync(id).Result ?? new SpedycjaCzynnosciMagazynowoSpedycyjneMag());
            }
            catch (Exception ex)
            {
                this.toastNotification.AddErrorToastMessage("Brak połączenia z bazą danych");
        ...
        public IActionResult Create(SpedycjaCzynnosciMagazynowoSpedycyjneMag model)
        {
            try
            {
                if (!this.CheckPermission(ModuleName))
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public IActionResult Delete(SpedycjaCzynnosciMagazynowoSpedycyjneMag model)
        {
            try
            {
                if (!this.CheckPermission(ModuleName))
        ...
        public IActionResult ExportExcel()
        {
            try
            {
                return ExcelExporter.Export(this.repository.GetAllAsync().Result?.ToList() ?? new List<SpedycjaCzynnosciMagazynowoSpedycyjneMag>(), ModuleName);
            }
            catch
            {
                this.toastNotification.AddErrorToastMessage("Brak połączenia z bazą danych");
