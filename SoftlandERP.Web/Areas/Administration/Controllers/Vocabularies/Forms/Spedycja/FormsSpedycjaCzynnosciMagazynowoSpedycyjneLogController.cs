...
{
    [Area("Administration")]
    public class FormsSpedycjaCzynnosciMagazynowoSpedycyjneLogController : BaseController
    {
        public static readonly string Module = "Słownik - ";
        public static readonly string Name = "Spedycja - Czynności Magazynowo-Spedycyjne Log";
        public static readonly string ModuleName = Module + Name;

        private readonly IRepository<SpedycjaCzynnosciMagazynowoSpedycyjneLog> repository;
        ...
        public FormsSpedycjaCzynnosciMagazynowoSpedycyjneLogController(IADRepository adRepository, IRepository<HelperText> helperTextVocabularyRepository, IToastNotification toastNotification, ILogger<BaseController> logger, IRepository<SpedycjaCzynnosciMagazynowoSpedycyjneLog> repository, IRepository<OgolneStan> stanRepository, IRepository<SpedycjaCzynnosci> czynnosciRepository, IRepository<OgolneKategoriaPracy> rodzajepracRepository)
        ...
        public IActionResult Create(Guid? id)
        {
            try
            {
                ...
                return this.PartialView("Modals/Create", this.repository.GetByIdAsync(id).Result ?? new SpedycjaCzynnosciMagazynowoSpedycyjneLog());
            }
            catch (Exception ex)
            {
                this.toastNotification.AddErrorToastMessage("Brak połączenia z bazą danych");
        ...
        public IActionResult Create(SpedycjaCzynnosciMagazynowoSpedycyjneLog model)
        {
            try
            {
                if (!this.CheckPermission(ModuleName))
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public IActionResult Delete(SpedycjaCzynnosciMagazynowoSpedycyjneLog model)
        {
            try
            {
                if (!this.CheckPermission(ModuleName))
        ...
        public IActionResult ExportExcel()
        {
            try
            {
                return ExcelExporter.Export(this.repository.GetAllAsync().Result?.ToList() ?? new List<SpedycjaCzynnosciMagazynowoSpedycyjneLog>(), ModuleName);
            }
            catch
            {
                this.toastNotification.AddErrorToastMessage("Brak połączenia z bazą danych");
