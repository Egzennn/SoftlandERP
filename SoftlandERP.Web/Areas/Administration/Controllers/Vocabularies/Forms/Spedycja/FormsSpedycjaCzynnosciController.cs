...
{
    [Area("Administration")]
    public class FormsSpedycjaCzynnosciController : BaseController
    {
        public static readonly string Module = "Słownik - ";
        public static readonly string Name = "Spedycja - Czynności";
        public static readonly string ModuleName = Module + Name;

        private readonly IRepository<SpedycjaCzynnosci> repository;
        private readonly IRepository<OgolneStan> stanRepository;

        public FormsSpedycjaCzynnosciController(IADRepository adRepository, IRepository<HelperText> helperTextVocabularyRepository, IToastNotification toastNotification, ILogger<BaseController> logger, IRepository<SpedycjaCzynnosci> repository, IRepository<OgolneStan> stanRepository)
        ...
        public IActionResult Create(Guid? id)
        {
            try
            {
                ...
                return this.PartialView("Modals/Create", this.repository.GetByIdAsync(id).Result ?? new SpedycjaCzynnosci());
            }
            catch (Exception ex)
            {
                this.toastNotification.AddErrorToastMessage("Brak połączenia z bazą danych");
        ...
        public IActionResult Create(SpedycjaCzynnosci model)
        {
            try
            {
                if (!this.CheckPermission(ModuleName))
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public IActionResult Delete(SpedycjaCzynnosci model)
        {
            try
            {
                if (!this.CheckPermission(ModuleName))
        ...
        public IActionResult ExportExcel()
        {
            try
            {
                return ExcelExporter.Export(this.repository.GetAllAsync().Result?.ToList() ?? new List<SpedycjaCzynnosci>(), ModuleName);
            }
            catch
            {
                this.toastNotification.AddErrorToastMessage("Brak połączenia z bazą danych");
        ...
        public async Task<ActionResult> AktualizujOdpowiedzialnego(string odpowiedzialnySelectList)
        {
            try
            {
                ...
                if (values?.Any() == true)
                {
                    ...
                    this.toastNotification.AddSuccessToastMessage("Powodzenie. Słownik został zmodyfikowany");
                }
                else
                {
                    this.toastNotification.AddErrorToastMessage("Bład przy próbie modyfikacji słownika");
                }
