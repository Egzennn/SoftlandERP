...
{
    [Area("Administration")]
    public class FormsDokumentyKosztObslugiController : BaseController
    {
        public static readonly string Module = "Słownik - ";
        public static readonly string Name = "Koszt obslugi";
        public static readonly string ModuleName = Module + Name;

        private readonly IRepository<DokumentyKosztObslugi> repository;
        private readonly IRepository<OgolneStan> stanRepository;

        public FormsDokumentyKosztObslugiController(IADRepository adRepository, IRepository<HelperText> helperTextVocabularyRepository, IToastNotification toastNotification, ILogger<BaseController> logger, IRepository<DokumentyKosztObslugi> repository, IRepository<OgolneStan> stanRepository)
        ...
        public IActionResult Create(Guid? id)
        {
            try
            {
                ...
                return this.PartialView("Modals/Create", this.repository.GetByIdAsync(id).Result ?? new DokumentyKosztObslugi());
            }
            catch (Exception ex)
            {
                this.toastNotification.AddErrorToastMessage("Brak połączenia z bazą danych");
        ...
        public IActionResult Create(DokumentyKosztObslugi model)
        {
            try
            {
                if (!this.CheckPermission(ModuleName))
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public IActionResult Delete(DokumentyKosztObslugi model)
        {
            try
            {
                if (!this.CheckPermission(ModuleName))
        ...
        public IActionResult ExportExcel()
        {
            try
            {
                return ExcelExporter.Export(this.repository.GetAllAsync().Result?.ToList() ?? new List<DokumentyKosztObslugi>(), ModuleName);
            }
            catch
            {
                this.toastNotification.AddErrorToastMessage("Brak połączenia z bazą danych");
