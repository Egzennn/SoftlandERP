...
namespace SoftlandERP.Web.Areas.Administration.Controllers.Vocabularies.Forms.Ogolne
{
    [Area("Administration")]
    public class FormsOgolneRecepturyNormWykonawczychController : BaseController
    {
        public static readonly string Module = "Słownik - ";
        public static readonly string Name1 = "Ogolne - ";
        public static readonly string Name2 = "Receptury norm wykonawczych";
        public static readonly string Name = Name1 + Name2;
        public static readonly string ModuleName = Module + Name;

        private readonly IRepository<OgolneRecepturyNormWykonawczych> repository;
        ...
        public FormsOgolneRecepturyNormWykonawczychController(IADRepository adRepository, IRepository<HelperText> helperTextRepository, IToastNotification toastNotification, ILogger<BaseController> logger, IRepository<OgolneRecepturyNormWykonawczych> repository, IRepository<OgolneRodzajPracy> rodzajpracyRepository, IRepository<OgolneStan> stanRepository)
        ...
        public IActionResult Create(Guid? id)
        {
            try
            {
                ...
                this.ViewBag.Odpowiedzialny = new SelectList(this.adRepository.GetAllADUserAcronyms());
                this.ViewBag.RodzajPracy = new SelectList(this.rodzajpracyRepository.GetAllAsync().Result?.Where(x => x.Stan == "Aktywny").OrderBy(x => x.Wartosc).Select(x => x.Wartosc).ToList());
                ...
                return this.PartialView("Modals/Create", this.repository.GetByIdAsync(id).Result ?? new OgolneRecepturyNormWykonawczych());
            }
            catch (Exception ex)
            {
                this.toastNotification.AddErrorToastMessage("Brak połączenia z bazą danych");
        ...
        public IActionResult Create(OgolneRecepturyNormWykonawczych model)
        {
            try
            {
                if (!this.CheckPermission(ModuleName))
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public IActionResult Delete(OgolneRecepturyNormWykonawczych model)
        {
            try
            {
                if (!this.CheckPermission(ModuleName))
        ...
        public IActionResult ExportExcel()
        {
            try
            {
                return ExcelExporter.Export(this.repository.GetAllAsync().Result?.ToList() ?? new List<OgolneRecepturyNormWykonawczych>(), ModuleName);
            }
            catch
            {
                this.toastNotification.AddErrorToastMessage("Brak połączenia z bazą danych");
