...
{
    [Area("Administration")]
    public class FormsOgolneRodzajPracyController : BaseController
    {
        public static readonly string Module = "Słownik - ";
        public static readonly string Name1 = "Ogolne - ";
        public static readonly string Name2 = "Rodzaj pracy";
        public static readonly string Name = Name1 + Name2;
        public static readonly string ModuleName = Module + Name;

        private readonly IRepository<OgolneRodzajPracy> repository;
        private readonly IRepository<OgolneStan> stanRepository;

        public FormsOgolneRodzajPracyController(IADRepository adRepository, IRepository<HelperText> helperTextVocabularyRepository, IToastNotification toastNotification, ILogger<BaseController> logger, IRepository<OgolneRodzajPracy> repository, IRepository<OgolneStan> stanRepository)
        ...
        public IActionResult Create(Guid? id)
        {
            try
            {
                ...
                return this.PartialView("Modals/Create", this.repository.GetByIdAsync(id).Result ?? new OgolneRodzajPracy());
            }
            catch (Exception ex)
            {
                this.toastNotification.AddErrorToastMessage("Brak połączenia z bazą danych");
        ...
        public IActionResult Create(OgolneRodzajPracy model)
        {
            try
            {
                if (!this.CheckPermission(ModuleName))
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public IActionResult Delete(OgolneRodzajPracy model)
        {
            try
            {
                if (!this.CheckPermission(ModuleName))
        ...
        public IActionResult ExportExcel()
        {
            try
            {
                return ExcelExporter.Export(this.repository.GetAllAsync().Result?.ToList() ?? new List<OgolneRodzajPracy>(), ModuleName);
            }
            catch
            {
                this.toastNotification.AddErrorToastMessage("Brak połączenia z bazą danych");
