...
{
    [Area("Administration")]
    public class ViewsZatrudnieniDzialController : BaseController
    {
        public static readonly string Module = "Widok - ";
        public static readonly string Name = "Dział";
        public static readonly string ModuleName = Module + Name;

        private readonly XLContext xlContext;

        public ViewsZatrudnieniDzialController(IADRepository adRepository, IRepository<HelperText> helperTextVocabularyRepository, IToastNotification toastNotification, ILogger<BaseController> logger, XLContext xlContext)
        ...
        public async Task<IActionResult> Index()
        {
            try
            {
                ...
                var acronyms = await this.xlContext.Database.SqlQuery<string>($"SELECT * FROM UDBS_Slownik.dbo.ZatrudnieniDzialVocabulary").ToListAsync().ConfigureAwait(true);
