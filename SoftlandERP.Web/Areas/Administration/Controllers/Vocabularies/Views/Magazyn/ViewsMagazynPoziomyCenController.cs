...
{
    [Area("Administration")]
    public class ViewsMagazynPoziomyCenController : BaseController
    {
        public static readonly string Module = "Widok - ";
        public static readonly string Name = "Poziomy cen";
        public static readonly string ModuleName = Module + Name;
        private readonly XLContext xlContext;

        public ViewsMagazynPoziomyCenController(IADRepository adRepository, IRepository<HelperText> helperTextVocabularyRepository, IToastNotification toastNotification, ILogger<BaseController> logger, XLContext xlContext)
        ...
        public async Task<IActionResult> Index()
        {
            try
            {
                ...
                var acronyms = await this.xlContext.Database.SqlQuery<string>($"SELECT DISTINCT * FROM UDBS_Slownik.dbo.MagazynPoziomyCenVocabulary").ToListAsync().ConfigureAwait(true);
