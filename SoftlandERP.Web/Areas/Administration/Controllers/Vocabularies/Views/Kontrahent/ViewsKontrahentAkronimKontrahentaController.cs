...
namespace SoftlandERP.Web.Areas.Administration.Controllers.Vocabularies.Views.Kontrahent
{
    [Area("Administration")]
    public class ViewsKontrahentAkronimKontrahentaController : BaseController
    {
        public static readonly string Module = "Widok - ";
        public static readonly string Name = "Akronimy kontrahentów";
        public static readonly string ModuleName = Module + Name;

        private readonly XLContext xlContext;

        public ViewsKontrahentAkronimKontrahentaController(IADRepository adRepository, IRepository<HelperText> helperTextVocabularyRepository, IToastNotification toastNotification, ILogger<BaseController> logger, XLContext xlContext)
        ...
        public async Task<IActionResult> Index()
        {
            try
            {
                ...
                var acronyms = await this.xlContext.Database.SqlQuery<string>($"SELECT DISTINCT * FROM UDBS_Slownik.dbo.KontrahentAkronimVocabulary").ToListAsync().ConfigureAwait(true);
