...
namespace SoftlandERP.Web.Areas.Forms.Controllers.Stanowisko
{
    ...
    public class StanowiskoListaWypFormController : BaseController
    {
        ...
        public StanowiskoListaWypFormController(IADRepository adRepository, IRepository<HelperText> helperTextRepository, IToastNotification toastNotification, ILogger<BaseController> logger, IRepository<StanowiskoListaWypForm> repository, IRepository<OgolneStan> stanRepository, IRepository<OgolneStatus> statusRepository, IRepository<OgolnePriorytet> priorytetRepository, IRepository<StanowiskoRodzajeWyp> rodzajewypRepository, XLContext xlContext)
        ...
        public IActionResult Create(Guid? id)
        {
            try
            {
                ...
                return this.PartialView("Modals/Create", this.repository.GetByIdAsync(id).Result ?? new StanowiskoListaWypForm());
            }
            catch (Exception ex)
            {
                this.toastNotification.AddErrorToastMessage("Brak połączenia z bazą danych");
        ...
        public IActionResult Create(StanowiskoListaWypForm model)
        {
            try
            {
                if (!this.CheckPermission(ModuleName))
                ...
                if (model != null)
                {
                    if (this.repository.GetByIdAsync(model.Id).Result != null)
                    {
                        ...
                        {
                            StanowiskoListaWypForm? newmodel = this.repository.GetByIdAsync(model.Id).Result ?? throw new Exception("Karta not found in db");
        ...
        public IActionResult Clone(Guid? id)
        {
            try
            {
                ...
                var oldValue = this.repository.GetByIdAsync(id).Result ?? new StanowiskoListaWypForm();

                this.ModelState.Clear();
                var newValue = new StanowiskoListaWypForm()
                {
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public IActionResult Delete(StanowiskoListaWypForm model)
        {
            try
            {
                if (!this.CheckPermission(ModuleName))
        ...
        public IActionResult ExportExcel()
        {
            try
            {
                return ExcelExporter.Export(this.repository.GetAllAsync().Result?.ToList() ?? new List<StanowiskoListaWypForm>(), Name);
            }
            catch
            {
                this.toastNotification.AddErrorToastMessage("Brak połączenia z bazą danych");
