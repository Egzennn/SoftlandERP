...
namespace SoftlandERP.Web.Areas.Forms.Controllers.Stanowisko
{
    ...
    public class StanowiskoOprogramowanieFormController : BaseController
    {
        ...
        private readonly IRepository<StanowiskoGrupaNazwaOprogramowania> grupanazwaRepository;
        private readonly IRepository<OprogramowanieInstalacja> instalacjaRepository;
        private readonly IRepository<OprogramowanieRola> rolaRepository;
        private readonly IOprogramowanieRepository oprogramowanieRepository;
        private readonly XLContext xlContext;

        public StanowiskoOprogramowanieFormController(IADRepository adRepository, IRepository<HelperText> helperTextRepository, IToastNotification toastNotification, ILogger<BaseController> logger, IRepository<StanowiskoOprogramowanieForm> repository, IRepository<OgolneStan> stanRepository, IRepository<OgolneStatus> statusRepository, IRepository<StanowiskoGrupaOprogramowania> grupaRepository, IRepository<StanowiskoGrupaNazwaOprogramowania> grupanazwaRepository, IRepository<OprogramowanieInstalacja> instalacjaRepository, IRepository<OprogramowanieRola> rolaRepository, IOprogramowanieRepository oprogramowanieRepository, XLContext xlContext)
            : base(adRepository, helperTextRepository, toastNotification, logger)
        {
            ...
            this.grupanazwaRepository = grupanazwaRepository;
            this.instalacjaRepository = instalacjaRepository;
            this.rolaRepository = rolaRepository;
            this.oprogramowanieRepository = oprogramowanieRepository;
        ...
        public IActionResult Create(Guid? id)
        {
            try
            {
                ...
                this.ViewBag.GrupaNazwa = new SelectList(this.grupanazwaRepository.GetAllAsync().Result?.Where(x => x.Stan == "Aktywny").OrderBy(x => x.Wartosc).Select(x => x.Wartosc).ToList() ?? new List<string?>());
                this.ViewBag.Instalacja = new SelectList(this.instalacjaRepository.GetAllAsync().Result?.Where(x => x.Stan == "Aktywny").OrderBy(x => x.Wartosc).Select(x => x.Wartosc).ToList() ?? new List<string?>());
                ...
                return this.PartialView("Modals/Create", this.repository.GetByIdAsync(id).Result ?? new StanowiskoOprogramowanieForm());
            }
            catch (Exception ex)
            {
                this.toastNotification.AddErrorToastMessage("Brak połączenia z bazą danych");
        ...
        public IActionResult Create(StanowiskoOprogramowanieForm model)
        {
            try
            {
                if (!this.CheckPermission(ModuleName))
                ...
                if (model != null)
                {
                    if (this.repository.GetByIdAsync(model.Id).Result != null)
                    {
                        ...
                        {
                            StanowiskoOprogramowanieForm? newmodel = this.repository.GetByIdAsync(model.Id).Result ?? throw new Exception("Karta not found in db");
        ...
        public IActionResult Clone(Guid? id)
        {
            try
            {
                ...
                var oldValue = this.repository.GetByIdAsync(id).Result ?? new StanowiskoOprogramowanieForm();

                this.ModelState.Clear();
                var newValue = new StanowiskoOprogramowanieForm()
                {
                    Grupa = oldValue.Grupa,
                    Nazwa = oldValue.Nazwa,
                    Wersja = oldValue.Wersja,
                    IDI = oldValue.IDI,
                    Instalacja = oldValue.Instalacja,
                    Rola = oldValue.Rola,
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public IActionResult Delete(StanowiskoOprogramowanieForm model)
        {
            try
            {
                if (!this.CheckPermission(ModuleName))
        ...
        public IActionResult ExportExcel()
        {
            try
            {
                return ExcelExporter.Export(this.repository.GetAllAsync().Result?.ToList() ?? new List<StanowiskoOprogramowanieForm>(), Name);
            }
            catch
            {
                this.toastNotification.AddErrorToastMessage("Brak połączenia z bazą danych");
        ...
        [HttpGet]
        public JsonResult GetNazwaByGrupa(string grupa)
        {
            try
            {
                if (!string.IsNullOrWhiteSpace(grupa))
                {
                    IEnumerable<SelectListItem> nazwa = new SelectList(this.oprogramowanieRepository.FindNazwaByGrupaAsync(grupa).Result?.Select(x => new SelectListItem { Value = x, Text = x }).ToList(), "Value", "Text");
