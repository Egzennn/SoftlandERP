...
{
    [Area("Forms")]
    public class StanowiskoObiektyITFormController : BaseController
    {
        public static readonly string Module = "Formularz - ";
        public static readonly string Name1 = "Stanowisko - ";
        public static readonly string Name2 = "Obiekty IT";
        public static readonly string Name = Name1 + Name2;
        public static readonly string ModuleName = Module + Name1 + Name2;

        private readonly IRepository<StanowiskoObiektyITForm> repository;
        private readonly IRepository<OgolneStan> stanRepository;
        private readonly IRepository<OgolneStatus> statusRepository;
        private readonly IRepository<OprogramowanieGrupaUslug> grupauslugRepository;
        private readonly IRepository<OprogramowanieRola> rolaRepository;
        private readonly IRepository<OprogramowanieObiektIT> obiektitRepository;
        private readonly XLContext xlContext;

        public StanowiskoObiektyITFormController(IADRepository adRepository, IRepository<HelperText> helperTextRepository, IToastNotification toastNotification, ILogger<BaseController> logger, IRepository<StanowiskoObiektyITForm> repository, IRepository<OgolneStan> stanRepository, IRepository<OgolneStatus> statusRepository, IRepository<OprogramowanieGrupaUslug> grupauslugRepository, IRepository<OprogramowanieRola> rolaRepository, IRepository<OprogramowanieObiektIT> obiektitRepository, XLContext xlContext)
            : base(adRepository, helperTextRepository, toastNotification, logger)
        {
            this.repository = repository;
            this.stanRepository = stanRepository;
            this.statusRepository = statusRepository;
            this.grupauslugRepository = grupauslugRepository;
            this.rolaRepository = rolaRepository;
            this.obiektitRepository = obiektitRepository;
        ...
        public IActionResult Create(Guid? id)
        {
            try
            {
                ...
                this.ViewBag.Obiekty = new SelectList(this.obiektitRepository.GetAllAsync().Result?.Where(x => x.Stan == "Aktywny").OrderBy(x => x.Wartosc).Select(x => x.Wartosc).ToList() ?? new List<string?>());
                this.ViewBag.RodzajObiektu = new SelectList(new List<string> { "konto", "usluga" });
                this.ViewBag.GrupaUslug = new SelectList(this.grupauslugRepository.GetAllAsync().Result?.Where(x => x.Stan == "Aktywny").OrderBy(x => x.Wartosc).Select(x => x.Wartosc).ToList() ?? new List<string?>());
                ...
                return this.PartialView("Modals/Create", this.repository.GetByIdAsync(id).Result ?? new StanowiskoObiektyITForm());
            }
            catch (Exception ex)
            {
                this.toastNotification.AddErrorToastMessage("Brak połączenia z bazą danych");
        ...
        public IActionResult Create(StanowiskoObiektyITForm model)
        {
            try
            {
                if (!this.CheckPermission(ModuleName))
                ...
                if (model != null)
                {
                    if (this.repository.GetByIdAsync(model.Id).Result != null)
                    {
                        ...
                        {
                            StanowiskoObiektyITForm? newmodel = this.repository.GetByIdAsync(model.Id).Result ?? throw new Exception("Karta not found in db");
        ...
        public IActionResult Clone(Guid? id)
        {
            try
            {
                ...
                var oldValue = this.repository.GetByIdAsync(id).Result ?? new StanowiskoObiektyITForm();

                this.ModelState.Clear();
                var newValue = new StanowiskoObiektyITForm()
                {
                    Rola = oldValue.Rola,
                    Obiekty = oldValue.Obiekty,
                    RodzajObiektu = oldValue.RodzajObiektu,
                    GrupaUslug = oldValue.GrupaUslug,
                    NazwaUslugi = oldValue.NazwaUslugi,
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public IActionResult Delete(StanowiskoObiektyITForm model)
        {
            try
            {
                if (!this.CheckPermission(ModuleName))
        ...
        public IActionResult ExportExcel()
        {
            try
            {
                return ExcelExporter.Export(this.repository.GetAllAsync().Result?.ToList() ?? new List<StanowiskoObiektyITForm>(), Name);
            }
            catch
            {
                this.toastNotification.AddErrorToastMessage("Brak połączenia z bazą danych");
