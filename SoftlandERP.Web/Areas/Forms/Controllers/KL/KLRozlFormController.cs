...
namespace SoftlandERP.Web.Areas.Forms.Controllers.KL
{
    ...
    public class KLRozlFormController : BaseController
    {
        ...
        public KLRozlFormController(IADRepository adRepository, IRepository<HelperText> helperTextRepository, IToastNotification toastNotification, ILogger<BaseController> logger, IRepository<KLRozlForm> repository, IRepository<OgolneStan> stanRepository, IRepository<OgolneStatus> statusRepository, XLContext xlContext)
        ...
        public IActionResult Create(Guid? id)
        {
            try
            {
                ...
                this.ViewBag.TakNie = new SelectList(new List<string> { "Tak", "Nie" });
                ...
                return this.PartialView("Modals/Create", this.repository.GetByIdAsync(id).Result ?? new KLRozlForm());
            }
            catch (Exception ex)
            {
                this.toastNotification.AddErrorToastMessage("Brak połączenia z bazą danych");
        ...
        public IActionResult Create(KLRozlForm model)
        {
            try
            {
                if (!this.CheckPermission(ModuleName))
                ...
                if (model != null)
                {
                    if (this.repository.GetByIdAsync(model.Id).Result != null)
                    {
                        ...
                        {
                            KLRozlForm? newmodel = this.repository.GetByIdAsync(model.Id).Result ?? throw new Exception("Karta not found in db");
        ...
        public IActionResult Clone(Guid? id)
        {
            try
            {
                ...
                var oldValue = this.repository.GetByIdAsync(id).Result ?? new KLRozlForm();

                this.ModelState.Clear();
                var newValue = new KLRozlForm()
                {
                    IDK = oldValue.IDK,
                    DniKredytowe = oldValue.DniKredytowe,
                    LimitKredytowy = oldValue.LimitKredytowy,
                    Waluta = oldValue.Waluta,
                    BlokadaTransakcji = oldValue.BlokadaTransakcji,
                    BlokadaZamownia = oldValue.BlokadaZamownia,
                    DataPotwierdzenia = oldValue.DataPotwierdzenia,
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public IActionResult Delete(KLRozlForm model)
        {
            try
            {
                if (!this.CheckPermission(ModuleName))
        ...
        public IActionResult ExportExcel()
        {
            try
            {
                return ExcelExporter.Export(this.repository.GetAllAsync().Result?.ToList() ?? new List<KLRozlForm>(), Name);
            }
            catch
            {
                this.toastNotification.AddErrorToastMessage("Brak połączenia z bazą danych");
