...
namespace SoftlandERP.Web.Areas.Forms.Controllers.KL
{
    ...
    public class KLTerminKorFormController : BaseController
    {
        ...
        public KLTerminKorFormController(IADRepository adRepository, IRepository<HelperText> helperTextRepository, IToastNotification toastNotification, ILogger<BaseController> logger, IRepository<KLTerminKorForm> repository, IRepository<OgolneStan> stanRepository, IRepository<OgolneStatus> statusRepository, XLContext xlContext)
        ...
        public IActionResult Create(Guid? id)
        {
            try
            {
                ...
                return this.PartialView("Modals/Create", this.repository.GetByIdAsync(id).Result ?? new KLTerminKorForm());
            }
            catch (Exception ex)
            {
                this.toastNotification.AddErrorToastMessage("Brak połączenia z bazą danych");
        ...
        public IActionResult Create(KLTerminKorForm model)
        {
            try
            {
                if (!this.CheckPermission(ModuleName))
                ...
                if (model != null)
                {
                    if (this.repository.GetByIdAsync(model.Id).Result != null)
                    {
                        ...
                        {
                            KLTerminKorForm? newmodel = this.repository.GetByIdAsync(model.Id).Result ?? throw new Exception("Karta not found in db");
        ...
        public IActionResult Clone(Guid? id)
        {
            try
            {
                ...
                var oldValue = this.repository.GetByIdAsync(id).Result ?? new KLTerminKorForm();

                this.ModelState.Clear();
                var newValue = new KLTerminKorForm()
                {
                    IDK = oldValue.IDK,
                    Dokument = oldValue.Dokument,
                    Kwota = oldValue.Kwota,
                    Waluta = oldValue.Waluta,
                    TerminPierwotny = oldValue.TerminPierwotny,
                    TerminSkorygowany = oldValue.TerminSkorygowany,
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public IActionResult Delete(KLTerminKorForm model)
        {
            try
            {
                if (!this.CheckPermission(ModuleName))
        ...
        public IActionResult ExportExcel()
        {
            try
            {
                return ExcelExporter.Export(this.repository.GetAllAsync().Result?.ToList() ?? new List<KLTerminKorForm>(), Name);
            }
            catch
            {
                this.toastNotification.AddErrorToastMessage("Brak połączenia z bazą danych");
