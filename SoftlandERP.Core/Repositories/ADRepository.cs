...
namespace SoftlandERP.Core.Repositories
{
    public class ADRepository : IADRepository
    {
        ...
        private readonly IRepository<ApplicationPermissionsRule> permissionRulesRepository;
        private readonly IUserAddressRepository userAddressRepository;

        public ADRepository(IOptions<ADConfiguration> adConfiguration, IRepository<ApplicationPermissionsRule> permissionRulesRepository, IUserAddressRepository userAddressRepository)
        {
            this.adConfiguration = adConfiguration.Value;
            this.permissionRulesRepository = permissionRulesRepository;
            ...
            try
            {
                ...
                foreach (var adUser in adUsers)
                {
                    ...
                    if (userprincipal?.GetUnderlyingObjectType() == typeof(DirectoryEntry))
                    {
                        ...
                        adUser.Address = this.userAddressRepository.FindAddressByCountryCodeAsync(entry?.Properties["c"]?.Value?.ToString() ?? string.Empty, entry?.Properties["l"]?.Value?.ToString() ?? string.Empty, entry?.Properties["streetAddress"]?.Value?.ToString() ?? string.Empty).Result ?? new UserAddress();
                    }
                }
        ...
        public bool CreateUser(ADUser? adUser)
        {
            try
            {
                ...
                if (adUser != null)
                {
                    ...
                    entry.Properties["c"].Value = adUser.Address?.TwoLetterISOCountryName;
                    entry.Properties["l"].Value = adUser.Address?.City;
                    entry.Properties["streetAddress"].Value = adUser.Address?.Street;
                    entry.Properties["department"].Value = adUser.Department;
